...
namespace NeonShooter
{
    static class Sound
    {
        ...
        public static void Load(ContentManager content)
        {
            ...
            explosions = Enumerable.Range(1, 8).Select(x => content.Load<SoundEffect>("Sounds/explosion-0" + x)).ToArray();
            shots = Enumerable.Range(1, 4).Select(x => content.Load<SoundEffect>("Sounds/shoot-0" + x)).ToArray();
            spawns = Enumerable.Range(1, 8).Select(x => content.Load<SoundEffect>("Sounds/spawn-0" + x)).ToArray();
        }
    }
}